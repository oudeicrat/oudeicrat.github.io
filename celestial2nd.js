...
var ANIMDISTANCE = 7, //0.035,  // Rotation animation threshold, ~2deg in radians
...
Celestial2nd.display = function(config) {
  ...
  //parent div with id Celestial2nd.iId+celestial-map or body
  ...
  if (cfg.controls === true && $("celestial-zoomin") === null) {
    d3.select(parentElement).append("input").attr("type", "button").attr("id", Celestial2nd.iId+"celestial-zoomin").attr("value", "\u002b").on("click", function () { zoomBy(1.25); return false; });
    d3.select(parentElement).append("input").attr("type", "button").attr("id", Celestial2nd.iId+"celestial-zoomout").attr("value", "\u2212").on("click", function () { zoomBy(0.8); return false; });
  }
  ...
  if ($("error") === null) d3.select("body").append("div").attr("id", Celestial2nd.iId+"error");
  ...
  function getWidth() {
	//console.log('called getWidth, window = ',window);
    var w = 0;
    if (isNumber(cfg.width) && cfg.width > 0) w = cfg.width;
    else if (parent) w = parent.getBoundingClientRect().width - margin[0] *2;
    else {
    	console.log('in getWidth, parent is false, looking at window = ',window);
		w = window.getBoundingClientRect().width - margin[0]*2;
	}
...
if (typeof module === "object" && module.exports) {
  ...
  module.exports = {
    Celestial2nd: function() { return Celestial2nd; },
...
//display settings form in div with id Celestial2nd.iId+"celestial-form"
function form(cfg) {
  ...
  col.append("input").attr("type", "number").attr("maxlength", "4").attr("max", "20000").attr("min", "0").attr("title", "Map width").attr("id", Celestial2nd.iId+"width").attr("value", config.width).on("change", resize);
  ...
  // graticules & planes 
  col = frm.append("div").attr("class", "col").attr("id", Celestial2nd.iId+"lines");
...
function geo(cfg) {
  ...
  var dtFormat = d3.time.format("%Y-%m-%d %H:%M:%S"),
      ...
      frm = d3.select("#"+Celestial2nd.iId+"celestial-form form").insert("div", "div#general").attr("id", Celestial2nd.iId+"loc");
  ...
  function go() {
    ...
    //console.log('called go, date = ',date);
    ...
    if (!isNaN(lon) && !isNaN(lat)) {
      ...
      var dtc = new Date(date.valueOf() - (timeZone - localZone) * 60000);
		//console.log('dtc = ',dtc);
		//console.log('('+dtc.getUTCHours()+':'+dtc.getUTCMinutes()+' UTC, offset '+dtc.getTimezoneOffset()+')');
		//console.log('timeZone = ',timeZone);
		//console.log('localZone = ',localZone);
		var horizontalInverse = horizontal.inverse(dtc, [90, 0], geopos);
		//console.log('horizontalInverse = ',horizontalInverse);
      zenith = Celestial2nd.getPoint(horizontalInverse, config.transform);
      zenith[2] = 0;
...
function exportSVG(fname) {
  var doc = d3.select("body").append("div").attr("id", Celestial2nd.iId+"d3-celestial-svg").attr("style", "display: none"),
      svg = d3.select("#"+Celestial2nd.iId+"d3-celestial-svg").append("svg"), //.attr("style", "display: none"),
...
Celestial2nd.exportSVG = function(callback) {
  if (!callback) return;
